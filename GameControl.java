...
public class GameControl {
	...
	public GameControl(){}
	public void CheckCrashGrav(SpaceObject space,SpaceShip ship, GravitationalObject grav){
		
		int distance;
		
		distance = ((grav.getX()) - (ship.getPosX()+20))*((grav.getX()) - (ship.getPosX()+20)) 
				+ ((grav.getY()) - (ship.getPosY()+20))*((grav.getY()) - (ship.getPosY()+20));
			if( distance <= 45*45){
				boom.play();
				space.cont.add(explosion);
				explosion.setBounds(ship.ship.getX(),ship.ship.getY(),80,68);
				ship.ship.setBounds(600,500,70,70);
				//space.life1--;
			}
